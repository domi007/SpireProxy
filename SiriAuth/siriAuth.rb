	...
	def prep_received_object(object)
		...
		if object["properties"] != nil
			...
			if object["properties"]["validationData"] !=nil && !object["properties"]["validationData"].empty?
				puts "[Injecting ValidationData...] #{object["properties"]["validationData"].unpack("\000")}"
				object["properties"]["validationData"] = plist_blob("#{validationDataKey}")
		
			end
			if object["properties"]["sessionValidationData"] !=nil && !object["properties"]["sessionValidationData"].empty?
				puts "[Injecting moar ValidationData...] #{object["properties"]["sessionValidationData"].unpack("\000")}"
				object["properties"]["sessionValidationData"] = plist_blob("#{validationDataKey}")
			end
...
class SiriGuzzoniConnection < SiriProxyConnection
	...
	def received_object(object)
#		pp object		
		if object["class"]== "CommandFailed" && object["properties"]["reason"] == "Not authenticated"
			puts "lejartak a kulcsok"
		end
...
class SiriProxy
	def initialize(pluginClasses=[])
		EventMachine.run do
			EventMachine::start_server('0.0.0.0', 3000, SiriIPhoneConnection) { |conn|
